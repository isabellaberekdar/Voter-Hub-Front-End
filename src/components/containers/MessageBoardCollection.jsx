...
class MessageBoardCollectionContainer extends Component {
  ...
  handleSubmit = event => {
    ...
    axios
      .post("https://voterhub.herokuapp.com/api/messages", {
        user: "asdass",
        text: "scc",
        messageboardID: 1
      })
  ...
  getMessageBoards = async () => {
    var res = await axios.get("https://voterhub.herokuapp.com/api/messages/messageboard")
              ...
              <ol>
                <form onSubmit={this.handleSubmit}>
                  {msgBoard.messages.map(message => (
                    <div>
                      <li>
                        {message.user}: {"\xa0\xa0\xa0\xa0\xa0\xa0\xa0"} {message.text}
                      </li>
                    </div>
                  ))}
                  <input
