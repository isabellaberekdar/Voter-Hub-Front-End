      ...
      for (let key in props.store.divisions) {
        if (props.store.divisions.hasOwnProperty(key)) {
          ...
          if (props.store.divisions[key].officeIndices) {
            for (let i = 0; i < props.store.divisions[key].officeIndices.length; i++) {
              for (
                let j = 0;
                j <
                props.store.offices[props.store.divisions[key].officeIndices[i]].officialIndices
                  .length;
                j++
              ) {
                officialCards.push(
                  <OfficialCard
                    office={props.store.offices[props.store.divisions[key].officeIndices[i]]}
                    official={
  ...
  return (
    <div>
      <div className='header'>
        <div className='centered'>
          ...
          <h3 className='find-rep-description'>
            Enter your full address below to find your representatives, how to contact them, their
            funding records, and their latest social media and news mentions!
          </h3>
          <SearchBar
      ...
      <div className='home-content'>
        ...
        <h2 id='sign-up-title'>
          Sign up to save your representatives, participate in discussions, and more.
        </h2>
        <div className='row'>
            ...
            <div className='container'>
              ...
              <div className='overlay'>
                <button className='text'>
                  <Link to='/official/ocd-division%2Fcountry%3Aus/0/0'>Go to President's Page</Link>
                </button>
              </div>
            </div>
          </div>
